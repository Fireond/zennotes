...
const INTERNAL_SVG_FRAGMENT_RE = /^#[^\s#]+$/;
const SVG_URL_FUNCTION_RE =
  /url\(\s*(?:"([^"]*)"|'([^']*)'|([^\s)'";]+))\s*\)/gi;
const tikzRenderTokens = new WeakMap<HTMLElement, object>();

function hasUnsafeSvgUrl(value: string): boolean {
  if (!/url\s*\(/i.test(value)) return false;
  let matchCount = 0;
  for (const match of value.matchAll(SVG_URL_FUNCTION_RE)) {
    matchCount += 1;
    const target = (match[1] ?? match[2] ?? match[3] ?? "").trim();
    if (!INTERNAL_SVG_FRAGMENT_RE.test(target)) return true;
  }
  // Treat malformed URL functions conservatively instead of leaving browser
  // parsing differences to decide whether a request is made.
  return matchCount === 0;
}

/**
 * Treat the SVG returned across the desktop bridge as untrusted markup.
 *
 * node-tikzjax normally emits plain SVG, but the source of a TikZ fence belongs
 * to the note and TeX supports several ways to add links and SVG specials. Keep
 * the SVG/filter vocabulary needed by real diagrams while dropping scripts,
 * HTML islands, event handlers, and unsafe URL attributes before the markup is
 * attached to the live document. `<use>` is safe once DOMPurify has validated
 * its href and is needed by some optimized SVG output.
 */
export function sanitizeTikzSvg(svg: string): string {
  ...
  // Work with an inert fragment, then return exactly one SVG root. Besides
  // making the insertion contract explicit, this prevents a compromised
  // bridge response from smuggling sibling HTML alongside an otherwise-valid
  // diagram.
  ...
  // SVG references generated by TikZ are local (`#glyph`, `#clip-1`, ...).
  // Drop everything else so an SVG cannot load a tracking image or navigate
  // to an external URL even though the app CSP permits HTTPS images.
  for (const element of [root, ...Array.from(root.querySelectorAll("*"))]) {
    element.removeAttribute("src");
    for (const attribute of ["href", "xlink:href"]) {
      const value = element.getAttribute(attribute);
      if (value !== null && !INTERNAL_SVG_FRAGMENT_RE.test(value.trim())) {
        element.removeAttribute(attribute);
      }
    }
    for (const attribute of Array.from(element.attributes)) {
      if (hasUnsafeSvgUrl(attribute.value)) {
...
export function tintTikzSvg(surface: HTMLElement): void {
...
async function renderTikzBlock(el: HTMLElement): Promise<void> {
  ...
  if (!source) return;
  const renderToken = {};
  tikzRenderTokens.set(el, renderToken);
  const isCurrentRender = (): boolean =>
    tikzRenderTokens.get(el) === renderToken &&
    el.getAttribute("data-tikz-source") === source;
  ...
  try {
    ...
    if (result.ok && result.svg) {
      ...
      if (!svg) {
        surface.innerHTML =
          '<pre class="zen-diagram-error">TikZ error: Renderer returned invalid SVG</pre>';
        return;
      }
