...
describe('vimHalfPageKeymap', () => {
  ...
  it('omits explicitly unbound half-page actions', () => {
    const bindings = vimHalfPageKeymap(true, { 'nav.halfPageDown': null })

    expect(bindings.map((binding) => binding.key)).toEqual(['Ctrl-u'])
  })
})
